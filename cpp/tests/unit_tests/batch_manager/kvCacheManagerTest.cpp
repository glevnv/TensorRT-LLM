...
template <typename T>
void writePatternToOffloadedBlocksGDS(const std::string& directory, int blockId,
                                      SizeType32  numPools,
                                      int blockSize, int mask)
{
    for (size_t poolIdx = 0; poolIdx < numPools; ++poolIdx)
    {
        std::string filename = directory + "/block_" + std::to_string(blockId) + "_pool_" + std::to_string(poolIdx) + ".bin";
...
void runPartialCopyTest()
{
    ...
    auto dirPath = fs::absolute("test_partial_copy_tmp");
    fs::create_directories(dirPath);
    std::string directory = dirPath.string();
